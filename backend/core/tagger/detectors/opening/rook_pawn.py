"""
Opening Rook Pawn Move tag detector.

Detects when a player pushes a rook pawn (a3, h3, a6, h6) during the
opening phase of the game. This is often a preparatory or fianchetto move.

Conditions:
- Move is a pawn move
- Destination is a3, h3, a6, or h6
- Fullmove number <= 15
...
def detect(ctx: TagContext) -> TagEvidence:
    ...
    # Gate 2: Destination is rook pawn square
    dest = move.to_square
    if dest in ROOK_PAWN_OPENING_TARGETS:
        gates_passed.append("rook_pawn_square")
        evidence["destination_square"] = chess.square_name(dest)
    else:
        gates_failed.append("rook_pawn_square")
