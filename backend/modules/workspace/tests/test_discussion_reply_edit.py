"""
Discussion reply edit/delete tests.
"""
...
@pytest.mark.asyncio
async def test_edit_and_delete_reply(session, event_bus):
    ...
    reply = await reply_service.edit_reply(
        EditReplyCommand(
            reply_id=reply.id,
            content="Updated",
            actor_id="user-1",
    ...
    assert await reply_repo.get_by_id(reply.id) is None
