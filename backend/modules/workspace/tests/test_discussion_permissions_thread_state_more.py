"""
Discussion thread state permission extra tests.
"""
...
async def seed_acl(permission: Permission) -> None:
    ...
    async with config.async_session_maker() as session:
        await create_study_node(session, "study-13", "owner-1")
        await grant_acl(session, "study-13", "user-2", permission, "owner-1")
