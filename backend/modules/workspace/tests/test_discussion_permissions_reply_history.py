"""
Discussion reply history permission tests.
"""
...
async def seed_acl(permission: Permission) -> None:
    ...
    async with config.async_session_maker() as session:
        await create_study_node(session, "study-10", "owner-1")
        await grant_acl(session, "study-10", "user-2", permission, "owner-1")
        await session.commit()


@pytest.mark.asyncio
async def test_reply_history_requires_commenter(app: FastAPI):
