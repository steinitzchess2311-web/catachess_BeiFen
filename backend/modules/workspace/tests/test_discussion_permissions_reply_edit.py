"""
Discussion reply edit permission tests.
"""
...
async def seed_acl(permission: Permission) -> None:
    ...
    async with config.async_session_maker() as session:
        await create_study_node(session, "study-4", "owner-1")
        await grant_acl(session, "study-4", "user-2", permission, "owner-1")
        await session.commit()


@pytest.mark.asyncio
async def test_edit_reply_requires_author(app: FastAPI):
    await init_test_db()
